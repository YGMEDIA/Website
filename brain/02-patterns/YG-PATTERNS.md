...
**Wann:** Eigene Produkte und das Partnerprojekt auf der Startseite. Seit 2026-09-19 ist das Raster der einzige Inhalt der Startseite: vier Karten USELY, YOU, Paukbox, Felgen Brillant, 4 Spalten ab 1200 px, 2 Spalten bis 768 px, darunter 1; Kartentitel als h2 unter einer unsichtbaren H1; Links bündig am Kartenboden (`.product-links` mit padding-top statt margin-top am Link). **Seit 2026-09-20 (Yasin):** Die Karte zeigt nur noch Bild, `.ref-tag` (ein Stichpunkt, was es ist, in `--teal-bright`), Produktname als h2, `.service-tags` und Links. Kein Beschreibungstext mehr (am 2026-09-19 war es kurz ein Absatz mit rund 200 Zeichen). Ab vier Spalten per Subgrid ausgerichtet (`grid-template-rows: subgrid; grid-row: span 5`, Stichpunkt `align-self: end`, Raster `row-gap: 0`, Titel ohne unteren Abstand), damit ein zweizeiliger Stichpunkt Titel und Chips nicht versetzt. Die Zahl in `span N` ist die Zahl der Kinder einer Karte: wer eins ergänzt oder streicht, zieht sie mit. Folge fürs Ranking: Die Startseite hat fast keinen Fließtext mehr, Produkttexte gehören ab jetzt auf die Produktseiten, nicht in die Karte. Kein Hero, keine Hero-Kacheln, keine Footer-Produktspalte, keine Fakten-Karte mehr. Der Rest dieses Eintrags beschreibt die Kartenanatomie und gilt weiter.
