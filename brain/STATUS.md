...
**Blockiert:** IONOS-CNAME (Domain) und GA4-Mess-ID (Tracking), beides Yasin, siehe "Braucht Yasin" 0b1/0b2.
**Ohne Yasin baubar:** EN-Version unter /en/, frische Screenshots aus dem Simulator (v1.2 mit Boards und Bezahllink), Lighthouse-Feinschliff.

## Nächster geplanter Schritt (frische Session)
0. **USELY One-Pager:** nach Yasins DNS-Umstellung die Custom Domain setzen (`gh api -X PUT repos/YGMEDIA/usely-site/pages -f cname=usely.yg-media.de -F https_enforced=true`), Live-Check, GSC-Property anlegen lassen. Danach GA4-ID eintragen, dann EN-Version und frische Simulator-Screenshots.
...
0b1. **IONOS-DNS für USELY (Blocker):** Beim Eintrag `usely` den A-Record 217.160.0.244 löschen und einen CNAME `usely` → `ygmedia.github.io` anlegen. Danach Bescheid geben, den Rest macht Claude Code.
0b2. **GA4 und Search Console (Blocker fürs Tracking):** Die Google-Sitzung in Chrome hat sich beim Property-Wechsel abgemeldet (Passwort-Prompt für info@yg-media.de; Passwörter gibt Claude Code nicht ein). Bitte einloggen, in Analytics einen Datenstream "USELY Website" für https://usely.yg-media.de anlegen und die Mess-ID durchgeben.
